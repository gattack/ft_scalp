...
Note on stoploss: the class-level `stoploss = -0.10` below is INERT — X6 and
X7's `confirm_trade_exit` deny stop_loss/trailing_stop_loss in both live and
backtest (only true liquidation crossings get through). The real loss floor
is implemented in NostalgiaScalpProX6X7Exp.custom_exit (backstop ladder for
non-exp trades, progressive cuts for exp_ trades).
...
class NostalgiaScalpProX6X7Opt(IStrategy):
    ...
    @staticmethod
    def version() -> str:
        return "26.04.28.14.22"
    can_short = False

    # Class-level stoploss is INERT — see module docstring. Kept at -0.10 so
    # freqtrade's startup config validation is happy; actual loss floor lives
    # in NostalgiaScalpProX6X7Exp.custom_exit (backstop ladder + exp_ cuts).
    stoploss = -0.10
