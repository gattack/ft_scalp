...
Performance optimization:
  ...
  Column reduction per timeframe:
    ...
    5m:  56 → 36 cols (drop 20, -36%)  ← base_tf_5m_indicators override
  Total: 201 → 123 cols, ~39% fewer indicator computations per pair.
  ...
  Additional drops in 5m base frame:
    RSI_4, RSI_20              (only referenced in disabled DCA grind functions)
    EMA_3, EMA_100             (EMA_3 unreferenced; EMA_100 only in disabled DCA)
    SMA_16, SMA_30             (only referenced in disabled DCA grind functions)
    BBM_20_2.0, BBP_20_2.0    (not referenced in any active entry/exit path)
    BB40 entire block          (ta.BBANDS length=40 + 7 derived cols — X6 base_tf
                                does NOT compute BB40, so X6 exits cannot use it)
    STOCHRSId_14_14_3_3 (5m)  (d-line unused, same as other timeframes)
...
class NostalgiaScalpProOptimized(NostalgiaForInfinityX7):
    ...
    long_entry_signal_params = {
        ...
        # Quick mode — tags 43/62 cut; 41/42/44/45/46 clean across 5 years
    ...
    # Long-only — shorts via tag 542 (short Quick mode, 100% win 5yr)
    ...
    def informative_1d_indicators(self, metadata: dict, info_timeframe) -> DataFrame:
        ...
        # Wicks (top + bot — bot needed by short condition 542)
        ...
        informative_1d["bot_wick_pct"] = (
            (np.minimum(informative_1d["open"], informative_1d["close"]) - informative_1d["low"])
    ...
    def informative_1h_indicators(self, metadata: dict, info_timeframe) -> DataFrame:
        ...
        # Min lows (low_min_12 needed by short condition 542)
    ...
    def base_tf_5m_indicators(self, metadata: dict, df: DataFrame) -> DataFrame:
        ...
        # RSI (4/20 only in disabled DCA; change_pcts not referenced in any active path)
        df["RSI_3"] = pta.rsi(df["close"], length=3)
        df["RSI_14"] = pta.rsi(df["close"], length=14)
        # EMA (3 unreferenced; 100 only in disabled DCA grind functions)
        ...
        # SMA (16/30 only in disabled DCA grind functions)
        df["SMA_9"] = pta.sma(df["close"], length=9)
        df["SMA_21"] = pta.sma(df["close"], length=21)
        df["SMA_200"] = pta.sma(df["close"], length=200)
        # BB 20 - STD2 (BBM/BBP not referenced in any active entry/exit path)
        ...
        # BB 40 entirely removed — X6 base_tf_5m_indicators does not compute BB40,
        # so X6 exit functions cannot reference it; X7 ScalpPro active paths don't use it either.
        # MFI
        ...
        # Stochastic RSI (k only — d-line unused in all active paths)
        ...
        # OBV removed — not referenced in any active ScalpPro entry/exit path
        # ROC
        ...
        # Global protections (identical to X7 — must be here since we override the method)
